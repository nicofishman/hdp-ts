...
const DroppableSection: FC<DroppableSectionProps> = ({
    ...
    return (
        <div
            className={clsx(
                'flex h-64 border-spacing-5 items-center justify-center rounded-md border-[3px] border-dashed border-gray-500 p-1 text-center transition-colors dark:border-gray-400',
                isOver && 'border-lime-500 dark:border-lime-500',
                currentPick > 1 && isHDP && 'w-96 gap-4',
                className
            )}
        ...
        >
            ...
            >
                        ...
                        {allPlayerSentTheirCards ? (
                            hdpDroppedCards.length === 0 ? (
                                ...
                                <>
                                    <div
                                        className={clsx(
                                            'absolute -top-1 -right-1 z-20 cursor-pointer rounded-full bg-red-500 p-1',
                                            currentPick > 1 && 'top-1'
                                        )}
                                        onClick={() =>
                                            undoCardHdp(
                ...
                {hasSentCards
                    ? game.sentCards
                          .filter((st) => st.playerId === user.uid)[0]
                          .cards.map((cardId: number) => (
                              <div key={cardId} className="relative">
                                  {!hasSentCards && (
                                      <div
                                          className="absolute -top-1 -right-1 z-20 cursor-pointer rounded-full bg-red-500 p-1"
                                          onClick={() => handleUndo(cardId)}
                                      >
                                          <ImCancelCircle color="#fff" />
                                      </div>
                                  )}
